...
module M_PolyChecker : M_PolyChecker = struct
    ...
    (* genlist * tvar *)
    type tscheme = T of (string list) * tvar
    ...
    let rec tv tvar =
        match tvar with
        | Prim _ -> SSet.empty
        | Arr(a,b) | Pair(a,b) -> SSet.union (tv a) (tv b)
        | Loc a -> tv a
        | Var v -> SSet.singleton v

    let ftv (T(gen,tvar)) =
        let ftv_tv = tv tvar in
        let pop r v = SSet.remove v r in
        List.fold_left pop ftv_tv gen

    module Gamma = struct (* TyEnv : id -> tscheme *)
        type id = string
        module G = Map.Make (
            struct
                type t = id
        ...
        let ftv gamma =
            let tschemes = (snd (List.split (G.bindings gamma))) in
            let f r v =
                SSet.union r (ftv v)
            in
            List.fold_left f SSet.empty tschemes
    end

    module Subst = struct (* Subst : tscheme -> tscheme *)
        module S = Map.Make (
            struct
                type t = string
                let compare = compare
        ...
        let subst_h (T(genlist,tvar)) subst =
            let vectorset = List.map (fun v -> (v,newvar ())) genlist in
            let gensubst = make vectorset in
            let t' = apply (apply tvar gensubst) subst in
            T(List.map (fun (_,Var v) -> v) vectorset,t')

        let subst_g gamma subst =
            Gamma.G.map (fun v -> subst_h v subst) gamma
    end
    ...
    let rec expansive exp =
        match exp with
        | APP _ | MALLOC _ -> true
        | BANG e | SEL1 e | SEL2 e | WRITE e -> expansive e
        | ASSIGN(e1,e2) | SEQ(e1,e2) | PAIR(e1,e2) | BOP(_,e1,e2) | LET(NREC(_,e1),e2) | LET(REC(_,e1),e2) -> (expansive e1) || (expansive e2)
        | CONST _ | VAR _ | READ | FN _ -> false
        | IF(e1,e2,e3) -> (expansive e1) || (expansive e2) || (expansive e3)

    let gen gamma tvar =
        let genset = SSet.diff (tv tvar) (Gamma.ftv gamma) in
        T(SSet.elements genset,tvar)

    let instantiation (T(genlist,tvar)) =
        Subst.apply tvar (Subst.make (List.map (fun v -> (v,newvar ())) genlist))
    ...
    let rec m gamma exp t (q:query) =
        ...
        | VAR id -> (unify (instantiation (Gamma.apply id gamma)) t,q)
        | FN(id,e) ->
                let a1 = newvar () in
                let a2 = newvar () in
                let s = unify (Arr(a1,a2)) t in
                let (s',q) = m (Gamma.add id (T([],(Subst.apply a1 s))) (Subst.subst_g gamma s)) e (Subst.apply a2 s) q in
                (Subst.subst_s s s',q)
        | APP(e1,e2) ->
                let a = newvar () in
        ...
        | LET(NREC(id,e),scope) ->
                ...
                let (s',q) =
                    if expansive e then
                        m (Gamma.add id (T([],(Subst.apply a s))) (Subst.subst_g gamma s)) scope (Subst.apply t s) q
                    else
                        m (Gamma.add id (gen (Subst.subst_g gamma s) (Subst.apply a s)) (Subst.subst_g gamma s)) scope (Subst.apply t s) q in
                (Subst.subst_s s s',q)
        | LET(REC(id,e),scope) ->
                let a = newvar () in
                let (s,q) = m (Gamma.add id (T([],a)) gamma) e a q in
                let (s',q) =
                    if expansive e then
                        m (Gamma.add id (T([],(Subst.apply a s))) (Subst.subst_g gamma s)) scope (Subst.apply t s) q
                    else
