...
struct
  ...
  let has elm lst = List.exists (fun y -> elm = y) lst
  let snds l = List.map (fun y -> snd y) l

  let rec trace mem addr visited = (* dfs location *)
      if has addr visited then visited else
          match (mem @? addr) with
          | Z _ -> addr::visited
          | B _ -> addr::visited
          | Unit -> addr::visited
          | L l ->
                  trace mem l (addr::visited)
          | R r ->
                  let tr vis (V (L lc)) = trace mem lc vis in
                  List.fold_left tr (addr::visited) (snds r)

  let rec eval_trace eval mem visited = (* dfs each e_val *)
      match eval with
      | (_, V(L l)) -> trace mem l visited
      | (_, P(_,_,env)) ->
              let tr vis ev = eval_trace ev mem vis in
              List.fold_left tr visited env
      | _ -> visited (* unreachable *)

  let env_trace env mem visited = (* dfs env *)
      let tr vis ev = eval_trace ev mem vis in
      List.fold_left tr visited env

  let rec sval_trace sval mem visited = (* dfs each svalue *)
      match sval with
      | V(L l) -> trace mem l visited
      | V(R r) ->
              let tr vis (V (L lc)) = trace mem lc vis in
              List.fold_left tr visited (snds r)
      | V(_) -> visited
      | M(_,sv) -> sval_trace sv mem visited
      | P(_,_,env) -> env_trace env mem visited

  let stack_trace stk mem visited =
      let tr vis sv = sval_trace sv mem vis in
      List.fold_left tr visited stk

  let konti_trace konti mem visited =
      let tr vis kv = env_trace (snd kv) mem vis in
      List.fold_left tr visited konti

  let trace mem env stk konti =
      let v = stack_trace stk mem [] in
      let v' = env_trace env mem v in
      konti_trace konti mem v'

  let rec filter mem vis res = (* return (loc, cell) list *)
      match mem with
      | [] -> res
      | (hd,tmp)::tl ->
              if List.exists (fun v -> (fst hd) = (fst v)) vis then (* add to loc *)
                  if List.exists (fun i -> i = (fst hd)) (snd res) then (* don't add to cell *)
                      filter tl vis (((hd,tmp)::(fst res)), snd res)
                  else filter tl vis (((hd,tmp)::(fst res)), ((fst hd)::(snd res)))
              else filter tl vis res

  let gc stk mem env konti =
      (* print_endline "GC CALL!"; *)
      let (filtered, cells) = filter mem (trace mem env stk konti) ([],[]) in
      let size = List.length cells in
      if size >= 128 then raise GC_Failure
      else
          malcount := size;
          filtered
  ...
  let rec eval (s,m,e,c,k) =
	eval(
     ...
     | (_,_,_,MALLOC::c,_) ->
             if !malcount >= 128 then
                 let m' = gc s m e k in
                 (V(L(newl()))::s, m', e, c, k)
             else (V(L(newl()))::s, m, e, c, k)
     | (_,_,_,BOX z::c,_) ->
        let rec box b i s =
			if i = 0 then V (R b)::s
			else
